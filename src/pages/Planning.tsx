...
import { ExternalLink, Link as LinkIcon, Save, Calendar } from 'lucide-react';
...
export default function PlanningPage() {
  ...
  useEffect(() => {
    if (settings) {
      if (settings.prorrateoLink) setDriveLink(settings.prorrateoLink);
      if (settings.fechaNotificacionProrrateo) setFechaNotificacion(settings.fechaNotificacionProrrateo);
    }
  }, [settings]);
  ...
  const handleSave = async () => {
    ...
    try {
      await db.settings.update(settings.id, { 
        prorrateoLink: driveLink,
        fechaNotificacionProrrateo: fechaNotificacion 
      });
  ...
  return (
    <div className="planning-container" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <div style={{ padding: '1rem', background: 'var(--accent)', borderRadius: '16px', color: 'white' }}>
          <Calendar size={32} />
        </div>
        <div>
          <h1 style={{ margin: 0 }}>Prorrateo Anual</h1>
          <p style={{ color: 'var(--text-muted)' }}>Gestión centralizada del prorrateo a través de Google Drive.</p>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginBottom: '1.5rem', fontSize: '0.9rem' }}>
          Pega aquí el enlace a tu Excel de prorrateo. De esta manera, podrás abrir y editar el documento original directamente sin tener que importarlo manualmente a la aplicación.
        </p>
      ...
      {driveLink && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem', background: 'var(--bg)', border: '2px dashed var(--accent)' }}>
          ...
          <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>
            Haz clic en el botón de abajo para abrir el Excel de prorrateo en una nueva pestaña y modificar los datos.
          </p>
          <button className="btn btn-primary" onClick={handleOpenLink} style={{ fontSize: '1.1rem', padding: '1rem 2rem', borderRadius: '50px' }}>
            Abrir Documento de Prorrateo <ExternalLink size={18} style={{ marginLeft: '0.5rem' }} />
          </button>
        </div>
      )}
    </div>
